#!/usr/bin/env python3
"""
AgriFun Foundation Architecture Demo (Windows Compatible)
=========================================================
...
Run with: python foundation_demo_win.py
"""
...
class EventSystem:
    ...
    def publish(self, event_type: str, event_data: Dict[str, Any], priority: EventPriority = EventPriority.NORMAL):
        ...
        if event_type in self.subscribers:
            print(f"[EVENT] Publishing {priority.value.upper()} event: {event_type}")
            for callback in self.subscribers[event_type]:
                try:
                    callback(event_data)
                except Exception as e:
                    print(f"[ERROR] Error in event handler: {e}")
...
class EntityComponentSystem:
    ...
    def create_entity(self, name: str = "") -> str:
        ...
        print(f"[ECS] Created entity: {entity_id} ({name})")
    ...
    def add_component(self, entity_id: str, component_type: ComponentType, component_data):
        ...
        print(f"[ECS] Added {component_type.value} component to {entity_id}")
    ...
    def update_component(self, entity_id: str, component_type: ComponentType, **updates):
        ...
        if entity_id in self.components[component_type]:
            ...
            print(f"[ECS] Updated {component_type.value} component for {entity_id}")
...
class ContentRegistry:
    ...
    def __init__(self, event_system: EventSystem):
        ...
        print("[CONTENT] Content Registry initialized")
    ...
    def _load_demo_content(self):
        ...
        print(f"[CONTENT] Loaded content: {len(crop_varieties)} crops, {len(equipment_data)} equipment, {len(employee_specializations)} specializations")
...
class AgriculturalSimulation:
    ...
    def __init__(self):
        ...
        print("\n[SUCCESS] Foundation systems initialized successfully!")
        print("=" * 60)
    
    def on_entity_created(self, event_data):
        """Handle entity creation events"""
        print(f"[HANDLER] Event Handler: New entity created - {event_data['entity_id']}")
    
    def on_component_added(self, event_data):
        """Handle component addition events"""
        print(f"[HANDLER] Event Handler: Component {event_data['component_type']} added to {event_data['entity_id']}")
    
    def on_crop_planted(self, event_data):
        """Handle crop planting events"""
        print(f"[HANDLER] Event Handler: {event_data['variety']} planted at ({event_data['x']}, {event_data['y']})")
    
    def on_employee_hired(self, event_data):
        """Handle employee hiring events"""
        print(f"[HANDLER] Event Handler: Hired {event_data['name']} as {event_data['specialization']}")
    
    def create_farm_scenario(self):
        """Create a demo farm scenario"""
        print("\n[FARM] Creating Demo Farm Scenario")
        print("-" * 40)
    ...
    def simulate_farm_operations(self, farm_entities: List[str]):
        ...
        print("\n[SIMULATE] Simulating Farm Operations")
        print("-" * 40)
    ...
    def demonstrate_content_system(self):
        ...
        print("\n[CONTENT] Content System Demonstration")
        print("-" * 40)
    ...
    def run_demo(self):
        ...
        try:
            ...
            print(f"Total entities created: {len(self.ecs.entities)}")
